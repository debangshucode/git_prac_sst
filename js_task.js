...
console.log(
  `does all nums are even - ${newArr.every((x) => {
    return x % 2 == 0;
  })}`,
);
console.log(
  `Atleast one is even - ${newArr.some((x) => {
    return x % 2 == 0;
  })}`,
);
...
console.log(`Q17 prevent error ${obj3?.Person?.Details?.Name}`)
...
console.log(`Q18 Default value when var is null : ${var2}`);
// 1️⃣9️⃣ Find the first number greater than 25 in an array and its index using find() and findIndex().
console.log(`new array - ${newArr}`);
let num25 = newArr.find((x) => {
  return x > 25;
});
let num25I = newArr.findIndex((x) => {
  return x > 25;
});
console.log(`Q19 The first number which is grater than 25 is -${num25}`);
console.log(`Q19 The index of the first num that is grater than 25 is : ${num25I}`);
// 2️⃣0️⃣ Flatten a deeply nested array into a single-level array using the flat() method.
let nArr = [1, [2, 3, [4, 5]], 6, [7, [8, [9]]]];
console.log(`Q20 flat- ${nArr.flat(Infinity)}`);
